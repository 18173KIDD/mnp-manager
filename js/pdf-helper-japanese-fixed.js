...
const PDFHelper = (() => {
    ...
    const generatePDF = async (container, selectedGroupId, getGroupFn) => {
        try {
            console.log('PDF生成処理開始 - HTML2Canvas方式 - レイアウト最適化版');
            ...
            // 幅を広げて横スクロールを防止
            hiddenContainer.style.width = '1000px'; // 広い固定幅
            ...
            hiddenContainer.innerHTML = `
                <style>
                    ...
                    #pdf-preview-table td {
                        ...
                        overflow: visible;
                        white-space: normal;
                        max-width: 200px; /* セル最大幅を設定 */
                    }
                    /* 列幅をカスタマイズ */
                    #pdf-preview-table th:nth-child(1), #pdf-preview-table td:nth-child(1) { width: 13%; } /* 電話番号 */
                    #pdf-preview-table th:nth-child(2), #pdf-preview-table td:nth-child(2) { width: 17%; } /* MNP予約番号 */
                    #pdf-preview-table th:nth-child(3), #pdf-preview-table td:nth-child(3) { width: 10%; } /* 移転元キャリア */
                    #pdf-preview-table th:nth-child(4), #pdf-preview-table td:nth-child(4) { width: 17%; } /* 乗り換え元名義 */
                    #pdf-preview-table th:nth-child(5), #pdf-preview-table td:nth-child(5) { width: 17%; } /* 乗り換え先名義 */
                    #pdf-preview-table th:nth-child(6), #pdf-preview-table td:nth-child(6) { width: 13%; } /* 利用者 */
                    #pdf-preview-table th:nth-child(7), #pdf-preview-table td:nth-child(7) { width: 13%; } /* 関係 */
            ...
            try {
                ...
                const canvas = await html2canvas(hiddenContainer, {
                    ...
                    windowWidth: 1000, // コンテナの幅と合わせる
                    width: 1000 // コンテナの幅と合わせる
                });
                ...
                // 画像サイズ計算（縦横比を維持しつつマージンを考慮）
                const imgWidth = availableWidth;
                const imgHeight = (canvas.height * imgWidth) / canvas.width;
                
                // 必要に応じてスケールダウン（高さがページに収まるように）
                ...
                // 画像をセンタリングして配置
                const imgX = margin + (availableWidth - finalImgWidth) / 2;
                const imgY = margin;
                
                // PDFに画像を追加
                doc.addImage(imgData, 'PNG', imgX, imgY, finalImgWidth, finalImgHeight);
     ...
     * @returns {Promise<Boolean>} 成功時はtrue、失敗時はfalseのPromise
     */
    const generateCSV = (selectedGroupId, getGroupFn, getGroupsFn) => {
        return new Promise((resolve, reject) => {
            try {
                ...
                // アラート表示を削除し、ログに記録するように変更
                // Storageオブジェクトが利用可能な場合はログに記録
                if (window.Storage && typeof window.Storage.logError === 'function') {
                    window.Storage.logError(error, 'CSV生成処理');
                }
