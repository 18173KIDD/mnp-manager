...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        showPreviewButton: document.getElementById('show-preview'),
                ...
                if (group.lines && group.lines.length > 0) {
                    ...
                    group.lines.forEach((line, index) => {
                        ...
                        lineRow.innerHTML = `
                            ...
                            <td>
                                <span class="editable-field" data-field="carrier" data-line-index="${index}" data-group-id="${group.id}">
                                    ${line.carrier || '-'}
                                </span>
                            </td>
                            <td>
                    ...
                    noLinesRow.innerHTML = `<td colspan="7">回線情報はありません</td>`;
    ...
    const updatePreview = () => {
        ...
        rows.forEach((row, index) => {
            try {
                ...
                const numberedSourceName = sourceName ? sourceName : '';
    ...
    const showSaveGroupModal = () => {
        try {
            const { customerNameInput, customerAgeInput } = elements;
            ...
            */
            ...
            console.error('グループ保存モーダル表示中にエラーが発生しました:', error);
            alert('グループ保存モーダルの表示中にエラーが発生しました。');
        }
    };
    ...
    const saveGroup = () => {
        try {
            ...
            const modalGroupNameInput = document.getElementById('modal-group-name');
            if (!modalGroupNameInput) {
                console.error('グループ名入力要素が見つかりません');
                return;
            }
            
            // グループ名を取得
            const groupName = modalGroupNameInput.value.trim();
            if (!groupName) {
    ...
    const updateGroupSelect = () => {
        ...
        try {
            ...
            // 選択肢をクリア
            groupSelect.innerHTML = '<option value="" disabled selected>グループを選択してください</option>';
            ...
            if (selectedGroupId) {
                ...
                if (group) {
                    ...
                    if (group.lines && group.lines.length > 0) {
                        group.lines.forEach((line, index) => {
                            const previewRow = document.createElement('tr');
                            const numberedSourceName = line.sourceName ? line.sourceName : '';
    ...
    const setupEventListeners = () => {
        const { savedGroupsToggle, savedGroupsPanel, customerNameInput, currentDateInput, customerAgeInput,
               ...
               clearAllButton, printTableButton, groupSelect, showPreviewButton, exportPdfButton, exportCsvButton } = elements;
               
        // エラーログダウンロードボタンのイベント設定
        const downloadErrorLogsButton = document.getElementById('download-error-logs');
        if (downloadErrorLogsButton) {
            downloadErrorLogsButton.addEventListener('click', () => {
                try {
                    // エラーログをダウンロード
                    if (Storage.downloadErrorLogs) {
                        const result = Storage.downloadErrorLogs();
                        if (!result) {
                            console.log('エラーログがありません');
                        }
                    } else {
                        console.error('エラーログダウンロード機能が見つかりません');
                    }
                } catch (error) {
                    console.error('エラーログダウンロード中にエラーが発生しました:', error);
                }
            });
        }
        ...
        // グループ保存ボタンのイベント
        if (saveGroupButton) {
            saveGroupButton.addEventListener('click', showSaveGroupModal);
        }
        
        // モーダル関連のイベントリスナーを設定
        const saveGroupModal = document.getElementById('save-group-modal');
        const modalCloseButton = document.querySelector('#save-group-modal .close');
        const confirmSaveButton = document.getElementById('confirm-save-group');
        const cancelSaveButton = document.getElementById('cancel-save-group');
        
        // 閉じるボタン
        if (modalCloseButton) {
            modalCloseButton.addEventListener('click', () => {
                if (saveGroupModal) saveGroupModal.style.display = 'none';
            });
        }
        
        // 保存確認ボタン
        if (confirmSaveButton) {
            confirmSaveButton.addEventListener('click', saveGroup);
        }
        ...
        window.addEventListener('click', (event) => {
            if (event.target === saveGroupModal) {
                saveGroupModal.style.display = 'none';
            }
        });
        ...
        if (showPreviewButton) {
            showPreviewButton.addEventListener('click', () => {
                try {
                    ...
                    // PDFヘルパーのプレビュー表示関数を使用
                    if (PDFHelper && typeof PDFHelper.showPreview === 'function') {
                        PDFHelper.showPreview(
                            elements.previewContainer,
                            elements.groupSelect.value,
                            Storage.getGroup
                        );
                    } else {
                        // フォールバック：従来のプレビュー更新関数を使用
                        updatePreviewForGroup();
                    }
        ...
        // プレビューを閉じるボタンのイベント
        const hidePreviewButton = document.getElementById('hide-preview');
        ...
        if (generatePdfButton) {
            generatePdfButton.addEventListener('click', () => {
                try {
                    ...
                    alert('PDF生成処理にエラーが発生しました: ' + error.message);
                }
            });
        }
        ...
        if (exportPdfButton) {
            exportPdfButton.addEventListener('click', () => {
                try {
                    ...
                    alert('PDF出力プレビュー表示中にエラーが発生しました: ' + error.message);
                }
            });
        }
                ...
                try {
                    ...
                    PDFHelper.generateCSV(elements.groupSelect.value, Storage.getGroup, Storage.getGroups)
                        ...
                        .catch(error => {
                            ...
                            // エラーログを保存（アラート表示の代わり）
                            if (Storage.logError) {
                                Storage.logError(error, 'CSV出力処理');
                            } else {
                                console.error('エラーロギング機能が見つかりません');
                            }
                        });
                } catch (error) {
                    ...
                    // エラーログを保存（アラート表示の代わり）
                    if (Storage.logError) {
                        Storage.logError(error, 'CSV出力処理の起動');
                    } else {
                        console.error('エラーロギング機能が見つかりません');
                    }
                }
